...
migrations = list((scan_root / 'migrations').rglob('*')) if (scan_root / 'migrations').exists() else []
...
code_tables = set()
for f in files:
    if any(x in str(f).lower() for x in ('venv', 'venv2', 'site-packages', 'backups', 'logs')):
        continue
    txt = _read_text(f)
...
stopwords = set(['api','app','flask','tests','test','postgres','psycopg2','import','from','select','where','join','on','and','or','in','not','as','is','set','table','columns','column','create','if','exists','return','def','class','current_timestamp','now'])
candidates = sorted([t for t in code_tables if len(t)>3 and t not in migration_tables and t not in stopwords and '_' in t])
