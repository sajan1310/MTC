...
def main():
    ...
    try:
        ...
        # Insert item_variant
        cur.execute(
            "INSERT INTO item_variant (item_id, color_id, size_id, opening_stock) VALUES (%s, %s, %s, 100) RETURNING variant_id;",
        ...
        cur.execute(
            ...
            (1, lot_number, user_id, "Planning", 10),
        )
        lot_id = cur.fetchone()[0]
        # Insert purchase_orders
        cur.execute(
            "INSERT INTO purchase_orders (po_number, supplier_id, order_date, status) VALUES (%s, %s, CURRENT_DATE, %s) RETURNING po_id;",
            (po_number, supplier_id, "Draft"),
        )
        po_id = cur.fetchone()[0]
        conn.commit()
        print(
            f"Inserted test data: variant_id={variant_id}, lot_id={lot_id}, user_id={user_id}, supplier_id={supplier_id}, po_id={po_id}"
        )
