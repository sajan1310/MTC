...
REPORT_FILE = os.path.abspath(os.path.join(os.path.dirname(__file__), '../schema_validation_report.txt'))
...
def test_sample_inserts(cur):
    try:
        ...
        cur.execute('SELECT variant_id FROM item_variant ORDER BY variant_id DESC LIMIT 1;')
        ...
        """, (variant_id, user_id))
        cur.execute("DELETE FROM inventory_alert_rules WHERE alert_rule_id = (SELECT MAX(alert_rule_id) FROM inventory_alert_rules);")
        ...
        """, (lot_id, variant_id))
        cur.execute("DELETE FROM production_lot_inventory_alerts WHERE alert_id = (SELECT MAX(alert_id) FROM production_lot_inventory_alerts);")
        ...
        cur.execute("DELETE FROM production_lot_procurement_recommendations WHERE recommendation_id = (SELECT MAX(recommendation_id) FROM production_lot_procurement_recommendations);")
...
def test_cascade_delete(cur):
    try:
        ...
        cur.execute(
            ...
            (1, f"LOT-VALID-{int(datetime.datetime.now().timestamp()*1000)}", user_id, 'Planning', 1)
        )
        ...
        cur.execute('SELECT COUNT(*) FROM production_lot_inventory_alerts WHERE production_lot_id = %s;', (new_lot_id,))
