    ...
    def _check_job_structure(self, workflow: dict, filename: str):
        ...
        for job_name, job_config in jobs.items():
            ...
            if "name" not in job_config:
                self.warnings.append(f"Job '{job_name}' missing descriptive name")
            ...
            if "steps" not in job_config:
                self.warnings.append(f"Job '{job_name}' has no steps defined")
...
def main():
    ...
    parser.add_argument(
        "--workflow", help="Specific workflow file to validate (e.g., ci.yml)"
    )
    parser.add_argument(
        "--workflow-dir",
