    ...
    try:
        ...
        with app.app_context():
            ...
            if not process:
                ...
                print("\n📝 Available processes:")
                cur.execute("SELECT id, name, status FROM processes ORDER BY id LIMIT 10")
            ...
            if not subprocesses:
                ...
                print("   This is OK - the process just doesn't have any subprocesses yet.")
            else:
                print(f"✅ Found {len(subprocesses)} subprocess(es):")
                for sp in subprocesses:
                    print(f"   - ID: {sp.get('id')}, Subprocess: {sp.get('subprocess_name', 'N/A')}")
            ...
            if check_table_exists(cur, 'additional_costs'):
                cur.execute("SELECT COUNT(*) as count FROM additional_costs WHERE process_id = %s", (process_id,))
            ...
            if check_table_exists(cur, 'profitability'):
                ...
                if profitability:
                    print(f"✅ Profitability data exists")
                else:
                    print(f"⚠️  No profitability data")
            ...
            print(f"{'✅' if subprocesses else '⚠️ '} Subprocesses: {len(subprocesses)} found")
            print()
            print("💡 CONCLUSION:")
            if not subprocesses:
                ...
                print("   Process structure looks good! If you're still seeing 500 errors,")
                print("   check the Flask server logs for the actual error message.")
            print()
            print("🧪 TO TEST THE ENDPOINT:")
            print(f"   curl http://localhost:5000/api/upf/processes/{process_id}/structure \\")
