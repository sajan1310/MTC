...
with app.app_context():
    try:
        print(
            "Fetching production lot via ProductionService.get_production_lot(%d)"
            % LOT_ID
        )
        ...
        if not lot:
            ...
            with get_conn() as (conn, cur):
                ...
                if subprocesses:
                    print(
                        "FIRST_SUBPROCESS=",
                        json.dumps(dict(subprocesses[0]), default=str, indent=2),
                    )
                    ...
                    if variants:
                        print(
                            "FIRST_VARIANT=",
                            json.dumps(dict(variants[0]), default=str, indent=2),
                        )
                else:
