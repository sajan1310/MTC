...
with get_conn() as (conn, cur):
    ...
    print('Column presence:', dict(po_has_po_number=po_has_po_number, po_has_notes=po_has_notes, sr_has_bill_number=sr_has_bill_number, sr_has_notes=sr_has_notes, se_has_cost=se_has_cost, se_has_qty=se_has_qty, se_has_variant=se_has_variant))
    ...
    po_select.append("po.po_number::text AS reference_number" if po_has_po_number else "NULL::text AS reference_number")
    po_select.extend([
    ...
    receipt_select.append("sr.bill_number::text AS reference_number" if sr_has_bill_number else "NULL::text AS reference_number")
    receipt_select.extend([
        "sr.receipt_id::bigint AS receipt_id",
        "se.entry_id::bigint AS stock_entry_id",
        "se.variant_id::bigint AS variant_id" if se_has_variant else "NULL::bigint AS variant_id",
        "se.quantity_added::numeric AS quantity" if se_has_qty else "NULL::numeric AS quantity",
        "se.cost_per_unit::numeric AS cost_per_unit" if se_has_cost else "NULL::numeric AS cost_per_unit",
