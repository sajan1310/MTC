...
def main():
    ...
    p.add_argument('--variant', type=str, help='Variant id to select (numeric or string)')
        ...
        try:
            ...
            if subs:
                for sp in subs:
                    ...
                    if sv:
                        first = sv[0]
                        vid = first.get('item_id') or first.get('variant_id') or first.get('usage_id') or first.get('id')
                        if vid:
                            chosen_variant = str(vid)
                            print('Auto-picked variant id from variant_options:', chosen_variant)
                            break
        except Exception:
            pass

    if not chosen_variant:
        print('No variant supplied and none auto-detected; skipping select-variant POST.')
        return
    ...
    payload = {
        ...
        'selected_variant_id': (int(chosen_variant) if chosen_variant.isdigit() else chosen_variant),
        'variant_id': (int(chosen_variant) if chosen_variant.isdigit() else chosen_variant),
