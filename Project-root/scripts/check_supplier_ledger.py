...
with get_conn() as (conn, cur):
    try:
        ...
        print(
            "Could not select from supplier_ledger (this may be expected if view not present):",
            e,
        )

with get_conn() as (conn, cur):
    cur.execute(
        "SELECT table_schema, table_name FROM information_schema.views WHERE table_name = 'supplier_ledger';"
    )
