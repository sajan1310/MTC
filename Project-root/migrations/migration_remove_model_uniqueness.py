...
def upgrade():
    ...
    with get_conn() as (conn, cur):
        ...
        if constraint_name:
            ...
            print(
                f"Upgrade complete: Dropped unique constraint '{constraint_name}' from model_master."
            )
        else:
            print(
                "No unique constraint found on model_master(model_name). No action needed."
            )
        conn.commit()
...
def downgrade():
    ...
    with get_conn() as (conn, cur):
        cur.execute(
            "ALTER TABLE model_master ADD CONSTRAINT model_master_model_name_key UNIQUE (model_name);"
        )
        print(
            "Downgrade complete: Re-added unique constraint to model_master(model_name)."
        )
        conn.commit()
