...
def upgrade():
    ...
    with get_conn() as (conn, cur):
        ...
        cur.execute("CREATE INDEX IF NOT EXISTS idx_processes_user_id ON processes(user_id);")
        cur.execute("CREATE INDEX IF NOT EXISTS idx_processes_status ON processes(status);")
        cur.execute("CREATE INDEX IF NOT EXISTS idx_processes_class ON processes(class);")
        cur.execute("CREATE INDEX IF NOT EXISTS idx_processes_deleted ON processes(is_deleted);")
        
        cur.execute("CREATE INDEX IF NOT EXISTS idx_subprocesses_reusable ON subprocesses(reusable);")
        cur.execute("CREATE INDEX IF NOT EXISTS idx_subprocesses_user_id ON subprocesses(user_id);")
        cur.execute("CREATE INDEX IF NOT EXISTS idx_subprocesses_deleted ON subprocesses(is_deleted);")
        
        cur.execute("CREATE INDEX IF NOT EXISTS idx_process_subprocesses_process_id ON process_subprocesses(process_id);")
        cur.execute("CREATE INDEX IF NOT EXISTS idx_process_subprocesses_subprocess_id ON process_subprocesses(subprocess_id);")
        cur.execute("CREATE INDEX IF NOT EXISTS idx_process_subprocesses_sequence ON process_subprocesses(process_id, sequence_order);")
        
        cur.execute("CREATE INDEX IF NOT EXISTS idx_variant_usage_process_subprocess ON variant_usage(process_subprocess_id);")
        cur.execute("CREATE INDEX IF NOT EXISTS idx_variant_usage_variant ON variant_usage(variant_id);")
        cur.execute("CREATE INDEX IF NOT EXISTS idx_variant_usage_substitute_group ON variant_usage(substitute_group_id);")
        
        cur.execute("CREATE INDEX IF NOT EXISTS idx_cost_items_process_subprocess ON cost_items(process_subprocess_id);")
        cur.execute("CREATE INDEX IF NOT EXISTS idx_additional_costs_process ON additional_costs(process_id);")
        
        cur.execute("CREATE INDEX IF NOT EXISTS idx_substitute_groups_process_subprocess ON substitute_groups(process_subprocess_id);")
        
        cur.execute("CREATE INDEX IF NOT EXISTS idx_variant_supplier_pricing_variant ON variant_supplier_pricing(variant_id);")
        cur.execute("CREATE INDEX IF NOT EXISTS idx_variant_supplier_pricing_supplier ON variant_supplier_pricing(supplier_id);")
        cur.execute("CREATE INDEX IF NOT EXISTS idx_variant_supplier_pricing_active ON variant_supplier_pricing(is_active, effective_from);")
        
        cur.execute("CREATE INDEX IF NOT EXISTS idx_process_worst_case_costing_process ON process_worst_case_costing(process_id);")
        
        cur.execute("CREATE INDEX IF NOT EXISTS idx_production_lots_process_id ON production_lots(process_id);")
        cur.execute("CREATE INDEX IF NOT EXISTS idx_production_lots_status ON production_lots(status);")
        cur.execute("CREATE INDEX IF NOT EXISTS idx_production_lots_user_id ON production_lots(user_id);")
        cur.execute("CREATE INDEX IF NOT EXISTS idx_production_lots_lot_number ON production_lots(lot_number);")
        
        cur.execute("CREATE INDEX IF NOT EXISTS idx_production_lot_selections_lot ON production_lot_selections(production_lot_id);")
        cur.execute("CREATE INDEX IF NOT EXISTS idx_production_lot_selections_group ON production_lot_selections(substitute_group_id);")
        
        cur.execute("CREATE INDEX IF NOT EXISTS idx_production_lot_actual_costing_lot ON production_lot_actual_costing(production_lot_id);")
        ...
        tables_with_updated_at = [
            'processes', 'subprocesses', 'variant_usage', 'cost_items',
            'process_timing', 'conditional_flags', 'profitability',
            'substitute_groups', 'variant_supplier_pricing',
            'process_worst_case_costing', 'production_lot_selections'
        ]
...
def downgrade():
    ...
    """
    with get_conn() as (conn, cur):
        print("WARNING: Starting downgrade - this will DELETE all process framework data!")
        ...
        conn.commit()
        print("✅ Downgrade complete: All process framework tables and triggers removed.")
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 1 and sys.argv[1] == 'downgrade':
        response = input("⚠️  WARNING: This will DELETE all process framework data. Type 'YES' to confirm: ")
        if response == 'YES':
            downgrade()
        else:
