...
def up():
    ...
    CREATE INDEX IF NOT EXISTS idx_variant_usage ON variant_usage (process_subprocess_id, variant_id);
        ...
        # Re-raise so callers (e.g. the test harness's multi-pass runner) can
        # retry after dependency tables exist, instead of recording this
        # migration as applied when it created nothing.
        raise
    finally:
        cur.close()
        conn.close()
