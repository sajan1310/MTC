...
def create_substitute_group():
    ...
    try:
        ...
        if not data.get("variant_ids") or len(data["variant_ids"]) < 2:
            return APIResponse.error(
                "validation_error", "At least 2 variant_ids required for OR group", 400
            )
...
@variant_api_bp.route(
    "/process_subprocess/<int:ps_id>/substitute_groups", methods=["GET"]
)
@login_required
...
@variant_api_bp.route(
    "/variant_usage/<int:usage_id>/remove_from_group", methods=["POST"]
)
@login_required
def remove_variant_from_group(usage_id):
    ...
    try:
        ...
        return APIResponse.success(
            dict(updated), "Variant removed from substitute group"
        )
    except Exception as e:
...
def remove_cost_item(cost_id):
    ...
    try:
        ...
        cur.execute(
            "UPDATE cost_items SET deleted_at = NOW() WHERE id = %s", (cost_id,)
        )
