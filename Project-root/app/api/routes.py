...
from .. import limiter
from ..utils import (
    role_required, get_or_create_master_id,
...
def _make_api_crud_routes(entity_name, table_name, id_col, name_col):
    ...
    def get_entities():
        with database.get_conn() as (conn, cur):
            query = sql.SQL('SELECT {}, {} FROM {} ORDER BY {}').format(
                sql.Identifier(id_col), sql.Identifier(name_col), sql.Identifier(table_name), sql.Identifier(name_col)
            )
            cur.execute(query)
            items = cur.fetchall()
    ...
    @api_bp.route(f'/{plural_name}/<int:item_id>', methods=['PUT'], endpoint=update_endpoint)
    @login_required
    @role_required('admin')
    ...
    @api_bp.route(f'/{plural_name}/<int:item_id>', methods=['DELETE'], endpoint=delete_endpoint)
    @login_required
    @role_required('admin')
...
def get_color_dependencies(color_id):
    try:
        ...
        current_app.logger.error(f"Error fetching dependencies for color {color_id}: {e}")
...
def get_model_dependencies(model_id):
    try:
        ...
        current_app.logger.error(f"Error fetching dependencies for model {model_id}: {e}")
        return jsonify({'error': 'Failed to fetch dependency count'}), 500
...
def get_variation_dependencies(variation_id):
    try:
        ...
        current_app.logger.error(f"Error fetching dependencies for variation {variation_id}: {e}")
        return jsonify({'error': 'Failed to fetch dependency count'}), 500
...
def get_supplier_contacts(supplier_id):
    try:
        ...
        current_app.logger.error(f"Error fetching contacts for supplier {supplier_id}: {e}")
...
def get_supplier_rates(supplier_id):
    try:
        ...
        current_app.logger.error(f"Error fetching rates for supplier {supplier_id}: {e}")
    ...
    try:
        with database.get_conn() as (conn, cur):
            ...
            for item in items:
                ...
                cur.execute('UPDATE item_variant SET opening_stock = opening_stock + %s WHERE variant_id = %s', (quantity, variant_id))
                if po_id:
                    cur.execute(
...
def get_stock_receipt_details(receipt_id):
    try:
        ...
        current_app.logger.error(f"Error fetching stock receipt details for {receipt_id}: {e}")
...
def get_purchase_orders():
    ...
    try:
        with database.get_conn(cursor_factory=psycopg2.extras.DictCursor) as (conn, cur):
            query = (
                'SELECT po.*, s.firm_name FROM purchase_orders po JOIN suppliers s ON po.supplier_id = s.supplier_id'
            )
            params = []
            conditions = []
            if status_filter:
...
def get_purchase_order_by_number(po_number):
    try:
        ...
        current_app.logger.error(f"Error fetching purchase order by number {po_number}: {e}")
        return jsonify({'error': 'Failed to fetch purchase order'}), 500
...
def get_variations_for_item_model():
    ...
    try:
        with database.get_conn() as (conn, cur):
            query = (
                'SELECT DISTINCT vm.variation_id, vm.variation_name FROM variation_master vm JOIN item_master im ON vm.variation_id = im.variation_id WHERE im.name = %s'
            )
            params = [item_name]
            if model_name:
        ...
        current_app.logger.error(f"Error fetching variations for item '{item_name}' and model '{model_name}': {e}")
...
def get_items():
    ...
    try:
        with database.get_conn(cursor_factory=psycopg2.extras.DictCursor) as (conn, cur):
            ...
            items = cur.fetchall()
            items_data = [{
                'id': item['item_id'], 'name': item['name'], 'model': item['model'], 'variation': item['variation'],
                'description': item['description'], 'image_path': item['image_path'],
                'threshold': int(item['total_threshold'] or 0), 'variant_count': item['variant_count'],
                'total_stock': int(item['total_stock'] or 0), 'has_low_stock_variants': item['has_low_stock_variants']
            } for item in items]
            return jsonify({'items': items_data, 'total_items': total_items, 'page': page, 'per_page': per_page, 'total_pages': (total_items + per_page - 1) // per_page})
    except Exception as e:
...
def add_item_api():
    ...
    if 'image' in request.files:
        ...
        if file.filename:
            try:
                validate_upload(file, user_id=current_user.id if hasattr(current_user, 'id') else None)
            except Exception as e:
...
def get_item(item_id):
    try:
        with database.get_conn(cursor_factory=psycopg2.extras.DictCursor) as (conn, cur):
            ...
            return jsonify({'id': item['item_id'], 'name': item['name'], 'model': item['model'], 'variation': item['variation'], 'description': item['description'], 'image_path': item['image_path']})
    except Exception as e:
...
def update_item(item_id):
    ...
    try:
        with database.get_conn(cursor_factory=psycopg2.extras.DictCursor) as (conn, cur):
            ...
            def get_master_id_with_cache(cache, value, table_name, id_col, name_col):
                ...
                try:
                    ...
                    cur.execute(sql.SQL('SELECT {} FROM {} WHERE {} = %s').format(sql.Identifier(id_col), sql.Identifier(table_name), sql.Identifier(name_col)), (value,))
                    result = cur.fetchone()
                    if result:
            ...
            model_id = get_master_id_with_cache(color_cache, data.get('model'), 'model_master', 'model_id', 'model_name')
            variation_id = get_master_id_with_cache(size_cache, data.get('variation'), 'variation_master', 'variation_id', 'variation_name')
            cur.execute(
            ...
            if cur.fetchone():
                return jsonify({'error': 'An item with these specifications already exists'}), 409
            update_fields = { 'name': data['name'], 'model_id': model_id, 'variation_id': variation_id, 'description': data.get('description') }
            if image_path:
                update_fields['image_path'] = image_path
            set_clause = sql.SQL(', ').join(sql.SQL('{} = %s').format(sql.Identifier(k)) for k in update_fields.keys())
            values = list(update_fields.values()) + [item_id]
            update_query = sql.SQL('UPDATE {} SET {} WHERE {} = %s').format(sql.Identifier('item_master'), set_clause, sql.Identifier('item_id'))
            ...
            for v in changed_variants.get('added', []):
                ...
                cur.execute('INSERT INTO item_variant (item_id, color_id, size_id, opening_stock, threshold, unit) VALUES (%s, %s, %s, %s, %s, %s)', (item_id, color_id, size_id, v.get('opening_stock', 0), v.get('threshold', 5), v.get('unit')))
            conn.commit()
            for v in changed_variants.get('updated', []):
                color_id = get_master_id_with_cache(color_cache, v['color'], 'color_master', 'color_id', 'color_name')
                size_id = get_master_id_with_cache(size_cache, v['size'], 'size_master', 'size_id', 'size_name')
                cur.execute('UPDATE item_variant SET color_id = %s, size_id = %s, opening_stock = %s, threshold = %s, unit = %s WHERE variant_id = %s', (color_id, size_id, v.get('opening_stock', 0), v.get('threshold', 5), v.get('unit'), v['id']))
            conn.commit()
            if changed_variants.get('deleted'):
                ...
                for id_val in changed_variants['deleted']:
                    try:
                        ...
                        current_app.logger.warning(f'Invalid variant ID for deletion: {id_val}')
                        continue
                if deleted_ids:
        ...
        return jsonify({'message': 'Item updated successfully', 'item': {
            'id': updated_item_data['item_id'], 'name': updated_item_data['name'], 'model': updated_item_data['model'], 'variation': updated_item_data['variation'], 'description': updated_item_data['description'], 'image_path': updated_item_data['image_path'], 'variant_count': updated_item_data['variant_count'], 'total_stock': int(updated_item_data['total_stock'] or 0), 'has_low_stock_variants': updated_item_data['has_low_stock_variants']
        }}), 200
    except psycopg2.IntegrityError as e:
...
def get_item_variants(item_id):
    try:
        with database.get_conn(cursor_factory=psycopg2.extras.DictCursor) as (conn, cur):
            ...
            return jsonify([{
                'id': v['variant_id'], 'color': {'id': v['color_id'], 'name': v['color_name']}, 'size': {'id': v['size_id'], 'name': v['size_name']}, 'opening_stock': v['opening_stock'], 'threshold': v['threshold'], 'unit': v['unit']
            } for v in variants])
    except Exception as e:
...
def update_variant_stock(variant_id):
    ...
    try:
        with database.get_conn() as (conn, cur):
            ...
            cur.execute('SELECT EXISTS (SELECT 1 FROM item_variant WHERE item_id = %s AND opening_stock <= threshold)', (item_id,))
            item_has_low_stock = cur.fetchone()[0]
            conn.commit()
            return jsonify({'message': 'Stock updated successfully', 'new_total_stock': int(total_stock or 0), 'item_has_low_stock': item_has_low_stock, 'updated_variant': {'stock': updated_stock, 'threshold': threshold, 'is_low_stock': updated_stock <= threshold}}), 200
    except Exception as e:
...
def update_variant_threshold(variant_id):
    ...
    try:
        with database.get_conn() as (conn, cur):
            cur.execute('UPDATE item_variant SET threshold = %s WHERE variant_id = %s', (int(new_threshold), variant_id))
            conn.commit()
        return jsonify({'message': 'Threshold updated successfully'}), 200
    except Exception as e:
        current_app.logger.error(f"Error updating threshold for variant {variant_id}: {e}")
        return jsonify({'error': 'Database error'}), 500
...
def add_variant(item_id):
    ...
    try:
        with database.get_conn() as (conn, cur):
            ...
            cur.execute('INSERT INTO item_variant (item_id, color_id, size_id, opening_stock, threshold, unit) VALUES (%s, %s, %s, %s, %s, %s) RETURNING variant_id', (item_id, color_id, size_id, data.get('opening_stock', 0), data.get('threshold', 5), data.get('unit')))
...
def update_variant(variant_id):
    ...
    try:
        with database.get_conn() as (conn, cur):
            color_id = get_or_create_master_id(cur, data['color'], 'color_master', 'color_id', 'color_name')
            size_id = get_or_create_master_id(cur, data['size'], 'size_master', 'size_id', 'size_name')
            cur.execute('UPDATE item_variant SET color_id = %s, size_id = %s, opening_stock = %s, threshold = %s, unit = %s WHERE variant_id = %s', (color_id, size_id, data.get('opening_stock', 0), data.get('threshold', 5), data.get('unit'), variant_id))
            conn.commit()
        ...
        with database.get_conn(cursor_factory=psycopg2.extras.DictCursor) as (conn, cur):
            ...
            def generate():
                ...
                for row in inventory_data:
                    writer.writerow([
                        row['item_name'], row['model_name'], row['variation_name'], row['color_name'], row['size_name'], row['opening_stock'], row['threshold'], row['unit']
                    ])
                    yield data.getvalue(); data.seek(0); data.truncate(0)
    ...
    try:
        with database.get_conn() as (conn, cur):
            ...
            for idx, row_data in enumerate(import_data, 1):
                ...
                if not item_name:
                    skipped_rows.append({'row_number': idx, 'reason': 'Missing item name'})
                    continue
                try:
                    ...
                    skipped_rows.append({'row_number': idx, 'item_name': item_name, 'reason': f'Invalid stock value: {str(e)}'})
                    continue
                processed += 1
                ...
                try:
                    ...
                    try:
                        ...
                        pass
                    failed_variants.append({'row_number': idx, 'item_name': item_name, 'color': color, 'size': size, 'error': 'Variant conflict or database error'})
                    current_app.logger.warning(f"Row {idx} variant insert failed for {item_name}/{color}/{size}: {e}")
            conn.commit()
        response = {
            'message': f"Import completed. Processed {processed} rows, imported {imported} variants.",
            'summary': {
                'total_rows': len(import_data), 'processed': processed, 'imported': imported, 'skipped': len(skipped_rows), 'failed': len(failed_variants)
            }
        }
        if skipped_rows:
        ...
        return jsonify({'error': 'Import failed due to a server error. Please contact support.'}), 500
