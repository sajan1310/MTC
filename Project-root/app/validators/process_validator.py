...
class ProcessValidator:
    ...
    VALID_PROCESS_CLASSES = ['assembly', 'manufacturing', 'packaging', 'quality_check', 'other']
    ...
    def validate_process_name(name: Optional[str]) -> str:
        ...
        if not re.match(r'^[a-zA-Z0-9\s\-_]+$', name):
            raise ValidationError(
                "Process name can only contain letters, numbers, spaces, hyphens, and underscores"
            )
    ...
    def validate_process_data(data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Optional fields
        validated['description'] = ProcessValidator.validate_description(data.get('description'))
        validated['process_class'] = ProcessValidator.validate_process_class(
            data.get('process_class', data.get('class', 'other'))
        )
        validated['status'] = ProcessValidator.validate_status(data.get('status', 'Active'))
...
class SubprocessValidator:
    ...
    VALID_COST_TYPES = ['labor', 'electricity', 'maintenance', 'tooling', 'overhead', 'other']
    ...
    def validate_variant_usage(data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if 'process_subprocess_id' not in data or not isinstance(data['process_subprocess_id'], int):
    ...
    def validate_cost_item(data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Required fields
        validated['cost_type'] = SubprocessValidator.validate_cost_type(data.get('cost_type'))
    ...
    def validate_substitute_group(data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if len(name) < 3 or len(name) > 200:
            raise ValidationError("Substitute group name must be between 3 and 200 characters")
        validated['name'] = name
