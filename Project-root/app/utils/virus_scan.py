...
if VIRUS_SCAN_ENABLED:
    try:
        ...
        if not cd.ping():
            logger.error(
                "[VIRUS_SCAN] ClamAV daemon not responding. Virus scanning disabled."
            )
...
def scan_file(file_path: str) -> Tuple[bool, str]:
    ...
    try:
        ...
        try:
            os.remove(file_path)
            logger.info(
                f"[VIRUS_SCAN] Deleted infected file: {os.path.basename(file_path)}"
            )
        except Exception as e:
        ...
        logger.error(
            f"[VIRUS_SCAN] Scan error | file={os.path.basename(file_path)} | error={str(e)}"
        )
...
def get_scan_status() -> dict:
    ...
    try:
        version = cd.version()
        stats = cd.stats()
        return {"enabled": True, "version": version, "stats": stats}
    except Exception as e:
        return {"enabled": False, "error": str(e)}


# Integration with file upload validation
def validate_upload_with_virus_scan(
    file_storage, max_size_mb: int = 10, user_id: int = None
) -> bool:
    """
