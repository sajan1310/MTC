...
class CostingService:
    ...
    @staticmethod
    def get_substitute_group_worst_case_cost(substitute_group_id: int) -> Optional[Dict[str, Any]]:
        """
        ...
        with database.get_conn(cursor_factory=psycopg2.extras.RealDictCursor) as (conn, cur):
            ...
            for variant in variants:
                variant_cost_info = CostingService.get_variant_worst_case_cost(variant['variant_id'])
                ...
                if variant_cost_info:
                    ...
                    alternatives_analysis.append({
                        ...
                        'worst_case_supplier_name': variant_cost_info['supplier_name'],
    ...
    def calculate_subprocess_cost(process_subprocess_id: int) -> Dict[str, Any]:
        ...
        for variant in fixed_variants:
            cost_info = CostingService.get_variant_worst_case_cost(variant['variant_id'])
            if cost_info:
        ...
        for group in substitute_groups:
            group_cost_info = CostingService.get_substitute_group_worst_case_cost(group['id'])
            if group_cost_info and group_cost_info['worst_case_cost'] > 0:
                ...
                quantity = next(
                    (alt['quantity'] for alt in group_cost_info['alternatives'] 
                ...
                substitute_group_costs.append({
                    ...
                    'worst_case_variant_id': group_cost_info['worst_case_variant_id'],
                    'worst_case_variant_name': group_cost_info['worst_case_variant_name'],
                    'quantity': quantity,
                    'total_cost': total_cost,
    ...
    def update_profitability(
        process_id: int,
        estimated_sales_price: Optional[float] = None
    ) -> Dict[str, Any]:
        """
        ...
        with database.get_conn(cursor_factory=psycopg2.extras.RealDictCursor) as (conn, cur):
            ...
            if existing:
                # Update existing
                sales_price = estimated_sales_price if estimated_sales_price is not None else existing.get('estimated_sales_price')
        ...
        return {
            'process_id': process_id,
            'total_worst_case_cost': float(profitability['total_worst_case_cost']) if profitability['total_worst_case_cost'] else 0,
            'estimated_sales_price': float(profitability['estimated_sales_price']) if profitability['estimated_sales_price'] else None,
            'profit_margin': float(profitability['profit_margin']) if profitability['profit_margin'] else None,
            'profit_amount': float(profitability['profit_amount']) if profitability['profit_amount'] else None,
            'last_calculated': profitability['last_calculated'].isoformat() if profitability['last_calculated'] else None,
            'is_profitable': (float(profitability['profit_amount']) > 0) if profitability['profit_amount'] else False,
        }
