...
import database
from ..models.process import VariantUsage, VariantSupplierPricing
...
class VariantService:
    ...
    def update_variant_usage(
        ...
        with database.get_conn(cursor_factory=psycopg2.extras.RealDictCursor) as (conn, cur):
            ...
            new_quantity = quantity if quantity is not None else float(current['quantity'])
            new_cost = cost_per_unit if cost_per_unit is not None else (float(current['cost_per_unit']) if current['cost_per_unit'] else None)
    ...
    def add_supplier_pricing(
        ...
        with database.get_conn(cursor_factory=psycopg2.extras.RealDictCursor) as (conn, cur):
            cur.execute("""
                ...
                RETURNING *
            """, (variant_id, supplier_id, cost_per_unit, currency,
                  minimum_order_qty, effective_from, effective_to))
    ...
    def search_variants(
        ...
        return [
            {
                **dict(v),
                'is_low_stock': float(v['opening_stock'] or 0) <= float(v['threshold'] or 0),
                'stock_status': 'in_stock' if float(v['opening_stock'] or 0) > 0 else 'out_of_stock',
            }
            for v in variants
        ]
