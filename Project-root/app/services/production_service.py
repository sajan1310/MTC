...
class ProductionService:
    ...
    def calculate_lot_actual_cost(lot_id: int) -> float:
        ...
        with database.get_conn(cursor_factory=psycopg2.extras.RealDictCursor) as (conn, cur):
            ...
            for variant in fixed_variants:
                ...
                if cost_info:
                    fixed_costs.append(cost_info['worst_case_cost'] * float(variant['quantity']))
            ...
            # Calculate total
            total = (sum(fixed_costs) + sum(selected_costs) + cost_items_total + additional_total) * lot['quantity']
    ...
    def execute_production_lot(lot_id: int) -> Dict[str, Any]:
        ...
        if not is_ready:
            raise ValueError(f"Lot not ready for execution. Missing selections: {', '.join(missing)}")
        ...
        with database.get_conn(cursor_factory=psycopg2.extras.RealDictCursor) as (conn, cur):
            ...
            for deduction in deductions:
                ...
                deduction['stock_after'] = deduction['stock_before'] - deduction['quantity']
    ...
    def get_variance_analysis(lot_id: int) -> Dict[str, Any]:
        ...
        variance = float(lot['actual_cost'] or 0) - float(lot['worst_case_estimated_cost'] or 0)
        variance_pct = (variance / float(lot['worst_case_estimated_cost']) * 100) if lot['worst_case_estimated_cost'] else 0
