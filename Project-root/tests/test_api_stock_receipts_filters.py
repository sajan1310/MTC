...
def test_stock_receipts_supplier_filter(authenticated_client):
    ...
    resp = authenticated_client.get('/api/stock-receipts?supplier_id=1&page=1&per_page=5')
...
def test_stock_receipts_variant_filter(authenticated_client):
    resp = authenticated_client.get('/api/stock-receipts?variant_id=1&page=1&per_page=5')
    assert resp.status_code in [200, 400, 401]
    if resp.status_code == 200:
        data = resp.get_json()
        assert 'items' in data
