...
def test_independent_axis_survives_name_collision_with_a_primary_color(erp_client):
    ...
    for color in ("Blue-Sky Blue", "Red-Black"):
        _rpc(erp_client, "saveWarehousePoolOpening", [{"processId": frame_id, "qty": 50, "color": color}], mutation=True)

    rim_payload, rim_id = _save_process(erp_client)
    for color in ("Black", "Chrome"):
        _rpc(erp_client, "saveWarehousePoolOpening", [{"processId": rim_id, "qty": 50, "color": color}], mutation=True)
    ...
    _save_lot(
        erp_client, down_id, frame_payload["outputItemName"],
        [
            # "Blue-Sky Blue" does not collide; "Red-Black" does. Pre-fix,
            # the second one alone was enough to strip the rim from BOTH.
            {"color": "Blue-Sky Blue", "qty": 7, "countsTowardTotal": True, "axisKey": frame_key},
            {"color": "Red-Black", "qty": 7, "countsTowardTotal": True, "axisKey": frame_key},
            {"color": "Black", "qty": 14, "countsTowardTotal": False, "axisKey": rim_key},
        ],
    )
...
def test_mirror_axis_without_axis_keys_still_folds_by_name(erp_client):
    ...
    assert _credited_buckets(erp_client, down_payload["outputItemName"]) == {"Blue-White": 10}
...
def test_pool_consumption_resolves_token_against_hyphenated_bucket(erp_client):
    ...
    for color in ("Pink-White", "Blue-White"):
        _rpc(erp_client, "saveWarehousePoolOpening",
             [{"processId": upstream_id, "qty": 50, "color": color}], mutation=True)
    ...
    down_payload, down_id = _save_process(
        ...
        components=[
            {"itemName": upstream_payload["outputItemName"], "qtyPerUnit": 1,
             "sourceType": "POOL", "colorGroup": "Pink"},
            {"itemName": "SeatPink", "qtyPerUnit": 1, "sourceType": "ITEM",
             "colorGroup": "Pink", "colorAxis": "Seat Color"},
            {"itemName": "SeatBlue", "qtyPerUnit": 1, "sourceType": "ITEM",
             "colorGroup": "Blue", "colorAxis": "Seat Color"},
        ],
    )
    resp = _rpc(erp_client, "saveProduction", [{
        "processId": down_id, "assignedTo": "Worker A", "status": "Completed",
        "primaryColorAxis": "Seat Color",
        "colorBreakdown": [{"color": "Pink", "qty": 5, "countsTowardTotal": True,
                            "axisKey": "tag:seat color"}],
        "componentsConsumed": [{"itemName": upstream_payload["outputItemName"], "qty": 5,
                                "sourceType": "POOL", "colorGroup": "Pink"}],
    }], mutation=True)
    assert resp.get_json()["success"] is True, resp.get_json()["message"]

    pool = _rpc(erp_client, "getWarehousePoolData").get_json()["data"]
    mine = [b for b in pool if b["outputItemName"] == upstream_payload["outputItemName"]]
    ...
    assert not any(b["color"] == "Pink" for b in mine), "phantom bare-token bucket was opened"
...
def test_consumption_only_bucket_is_attributed_to_its_producing_process(erp_client):
    ...
    upstream_payload, upstream_id = _save_process(erp_client)
    _rpc(erp_client, "saveWarehousePoolOpening",
         [{"processId": upstream_id, "qty": 10, "color": "Black"}], mutation=True)
    ...
    down_payload, down_id = _save_process(
        erp_client,
        components=[{"itemName": upstream_payload["outputItemName"], "qtyPerUnit": 1,
                     "sourceType": "POOL", "colorGroup": "Grey"}],
    )
    resp = _rpc(erp_client, "saveProduction", [{
        "processId": down_id, "assignedTo": "Worker A", "status": "Completed",
        "colorBreakdown": [{"color": "Grey", "qty": 4, "countsTowardTotal": True}],
        "componentsConsumed": [{"itemName": upstream_payload["outputItemName"], "qty": 4,
                                "sourceType": "POOL", "colorGroup": "Grey"}],
    }], mutation=True)
    assert resp.get_json()["success"] is True, resp.get_json()["message"]

    pool = _rpc(erp_client, "getWarehousePoolData").get_json()["data"]
    grey = next(b for b in pool
                if b["outputItemName"] == upstream_payload["outputItemName"] and b["color"] == "Grey")
    assert grey["consumedQty"] == 4
    assert grey["availableQty"] == -4
    assert grey["processId"] == upstream_id, "debit-only bucket must name its producing process"
...
def test_component_scoped_to_hyphen_segment_of_lot_colour_is_kept(erp_client):
    ...
    resp = _rpc(erp_client, "saveProduction", [{
        "processId": down_id, "assignedTo": "Worker A", "status": "Completed",
        "colorBreakdown": [{"color": "Pink-White", "qty": 6, "countsTowardTotal": True,
                            "axisKey": _axis_key(upstream_payload["outputItemName"])}],
        "componentsConsumed": [{"itemName": upstream_payload["outputItemName"], "qty": 6,
                                "sourceType": "POOL", "colorGroup": "Pink"}],
    }], mutation=True)
    body = resp.get_json()
    assert body["success"] is True, body["message"]
    ...
    pool = _rpc(erp_client, "getWarehousePoolData").get_json()["data"]
    pink = next(b for b in pool
                if b["outputItemName"] == upstream_payload["outputItemName"] and b["color"] == "Pink-White")
...
def test_component_scoped_to_a_colour_the_lot_did_not_make_is_still_dropped(erp_client):
    ...
    _down_payload, down_id = _save_process(
        ...
        components=[
            {"itemName": upstream_payload["outputItemName"], "qtyPerUnit": 1,
             "sourceType": "POOL", "colorGroup": "Pink"},
            {"itemName": "RawMat", "qtyPerUnit": 1, "sourceType": "ITEM", "colorGroup": "COMMON"},
        ],
    )
    resp = _rpc(erp_client, "saveProduction", [{
        "processId": down_id, "assignedTo": "Worker A", "status": "Completed",
        "colorBreakdown": [{"color": "Pink-White", "qty": 6, "countsTowardTotal": True,
                            "axisKey": _axis_key(upstream_payload["outputItemName"])}],
        "componentsConsumed": [
            {"itemName": upstream_payload["outputItemName"], "qty": 6,
             "sourceType": "POOL", "colorGroup": "Pink"},
            # "Blue" shares no segment with "Pink-White" -- must be dropped.
            {"itemName": "WrongColourPart", "qty": 3, "sourceType": "ITEM", "colorGroup": "Blue"},
...
def _common_consumer(erp_client, upstream_payload, lot_colour, consume_qty):
    ...
    _down_payload, down_id = _save_process(
        erp_client,
        components=[{"itemName": upstream_payload["outputItemName"], "qtyPerUnit": 1,
                     "sourceType": "POOL", "colorGroup": "COMMON"}],
    )
    resp = _rpc(erp_client, "saveProduction", [{
        "processId": down_id, "assignedTo": "Worker A", "status": "Completed",
        "colorBreakdown": [{"color": lot_colour, "qty": 1, "countsTowardTotal": True,
                            "axisKey": _axis_key(upstream_payload["outputItemName"])}],
        "componentsConsumed": [{"itemName": upstream_payload["outputItemName"],
                                "qty": consume_qty, "sourceType": "POOL", "colorGroup": "COMMON"}],
    }], mutation=True)
    body = resp.get_json()
    assert body["success"] is True, body["message"]
    pool = _rpc(erp_client, "getWarehousePoolData").get_json()["data"]
    return {b["color"]: b for b in pool if b["outputItemName"] == upstream_payload["outputItemName"]}


def test_common_consumption_drains_coloured_buckets_when_there_is_no_blank_credit(erp_client):
...
def test_common_consumption_beyond_all_colour_stock_stays_visible_as_negative(erp_client):
...
def test_common_consumption_still_takes_a_real_blank_bucket_first(erp_client):
    ...
    resp = _rpc(erp_client, "saveProduction", [{
        "processId": down_id, "assignedTo": "Worker A", "qty": 5, "status": "Completed",
        "componentsConsumed": [{"itemName": upstream_payload["outputItemName"], "qty": 12,
                                "sourceType": "POOL", "colorGroup": "COMMON"}],
    }], mutation=True)
    assert resp.get_json()["success"] is True, resp.get_json()["message"]

    pool = _rpc(erp_client, "getWarehousePoolData").get_json()["data"]
    blank = next(b for b in pool
