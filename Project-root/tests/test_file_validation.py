...
class TestFileValidation:
    ...
    @pytest.mark.skipif(sys.platform.startswith('win'), reason='libmagic on Windows may report CSV as text/plain')
...
class TestSecurityRegression:
    ...
    def test_no_fallback_import(self):
        ...
        import os
        file_path = os.path.join(os.path.dirname(__file__), '..', 'app', 'utils', 'file_validation.py')
        with open(file_path, 'r') as f:
            content = f.read()
            assert 'except ImportError' not in content, "Fallback import logic found - python-magic must be hard dependency"
