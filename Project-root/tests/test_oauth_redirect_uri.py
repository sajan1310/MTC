...
def test_oauth_redirect_uri_matches_expected(app, client):
    ...
    with app.test_request_context():
        ...
        assert (
            "auth.auth_google_callback" in all_endpoints
        ...
        assert redirect_uri.startswith("http://") or redirect_uri.startswith(
            "https://"
        ), f"Redirect URI must be absolute, got: {redirect_uri}"

        # Verify it contains the callback path
        assert (
            "/auth/google/callback" in redirect_uri
...
def test_oauth_initiate_returns_redirect_to_google(client):
    ...
    ), f"Should redirect to Google, got status {response.status_code}"
...
def test_callback_route_handles_missing_code(client):
    ...
    assert (
        b"Missing authorization code" in response.data
        or b"code" in response.data.lower()
    )
...
def test_config_has_oauth_credentials(app):
    ...
    assert "GOOGLE_CLIENT_ID" in app.config, "GOOGLE_CLIENT_ID not in app.config"
    assert (
        "GOOGLE_CLIENT_SECRET" in app.config
