...
class TestImportServiceBatching:
    ...
    def test_single_batch_processing(self):
        ...
        with patch('app.services.import_service.get_conn') as mock_conn:
            ...
            mock_conn.return_value.__enter__.return_value = (mock_connection, mock_cursor)
    ...
    def test_progress_callback_invocation(self):
        ...
        def track_progress(processed, total, percentage):
            progress_calls.append({'processed': processed, 'total': total, 'pct': percentage})
...
class TestImportServiceErrorHandling:
    ...
    def test_individual_row_failure_continues_batch(self):
        ...
        with patch('app.services.import_service.get_conn') as mock_conn:
            ...
            with patch.object(DataValidator, 'validate_batch', return_value=(data, [])):
                with patch.object(service, '_import_single_row', side_effect=side_effect_import):
                    result = service.import_items_chunked(data)
...
class TestDataValidatorEdgeCases:
    ...
    def test_validate_batch_type_coercion(self):
        ...
        data = [
            {'Item': 'Test', 'Stock': '100', 'Color': 'Red', 'Size': 'M'},  # String stock
        ]
        ...
        assert isinstance(valid[0].get('Stock'), (int, str))  # Depending on validator impl
...
class TestImportServiceIntegration:
    ...
    def test_import_with_mixed_results(self):
        ...
        all_data = [
            ...
            {'Item': 'ProcessFail', 'Stock': 300, 'Color': 'Green', 'Size': 'S'},  # Will fail processing
        ]
        ...
        with patch('app.services.import_service.get_conn') as mock_conn:
            ...
            with patch.object(DataValidator, 'validate_batch', return_value=(valid_data, invalid_data)):
                with patch.object(service, '_import_single_row', side_effect=import_side_effect):
