...
App.Item = {
  ...
  // The assembler moved into print-templates.js so MApp can reach it --
  // it joins six collections, which is why the phone never had this
  // document even though every one of them was reachable there.
  getLedgerData(name) {
    return PrintTemplates.itemLedgerSections(name, {
      items: App.State.globalItems,
      stock: App.State.globalStock,
      vendors: App.State.globalVendors,
      pos: App.State.globalPOs,
      bills: App.State.globalBills,
      itemLedgers: App.State.itemLedgers
    }, {
      ...App.Print.templateDeps(),
      getPendingByItem: () => App.Utils.getPendingByItem()
    });
  },
