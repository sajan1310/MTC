...
describe('every wired list sends the payload its RPC actually expects', () => {
  ...
  test('every wired list both declares a config and arms it', () => {
    ...
    expect(declared).toBe(12); // Directory builds its config per type, so it is not a SELECT literal
    ...
    // PI / Estimates, Returns, and the Admin user list.
    expect(armed).toBe(17);
  });
});
