...
print("\nRoute categories:")
print(
    f"  - Authentication routes: {sum(1 for r in routes if '/auth/' in r.get('full_path', ''))}"
)
print(f"  - API routes: {sum(1 for r in routes if '/api/' in r.get('full_path', ''))}")
print(
    f"  - UI/Page routes: {sum(1 for r in routes if '/api/' not in r.get('full_path', '') and '/auth/' not in r.get('full_path', ''))}"
)
print(
    f"  - Static routes: {sum(1 for r in routes if '/static/' in r.get('full_path', ''))}"
)
