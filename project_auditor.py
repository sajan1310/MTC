...
class FlaskProjectAuditor:
    ...
    def scan_directory_structure(self) -> Dict[str, Any]:
        ...
        excluded_dirs = {
            '__pycache__', 'venv', 'venv2', 'env', 'node_modules', 
            '.git', '.vscode', '.idea', 'dist', 'build', '.pytest_cache'
        }
                ...
                try:
                    ...
                    for node in ast.walk(tree):
                        if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)):
                            ...
                            elif any(isinstance(stmt, ast.Expr) and 
                                   isinstance(stmt.value, ast.Constant) and
                                   'TODO' in str(stmt.value.value) 
                                   for stmt in body):
                                incomplete.append({
                                    "file": str(py_file.relative_to(self.project_root)),
                                    "function": node.name,
    ...
    def extract_javascript_api_calls(self) -> List[Dict[str, Any]]:
        ...
        for js_file in all_files:
            if any(excluded in str(js_file) for excluded in ['venv', 'node_modules', '.git']):
                continue
            ...
            try:
                ...
                self.log_error(f"Error extracting API calls from {js_file}: {str(e)}", str(js_file))
    ...
    def compare_routes_and_calls(self) -> List[Dict[str, Any]]:
        ...
        for call in self.results["javascript_api_calls"]:
            ...
            for route, methods in route_map.items():
                ...
                if route == normalized_path or self._route_matches(route, normalized_path):
                    route_found = True
        ...
        for route in self.results["flask_routes"]:
            ...
            if normalized not in used_routes and not any(
                ...
                if not any(special in normalized for special in ['/static/', '/admin/', '/_']):
                    mismatches.append({
                        "type": "unused_route",
    ...
    def generate_statistics(self):
        ...
        self.results["statistics"] = {
            "total_python_files": self.results["directory_structure"].get("python_files", 0),
            "total_javascript_files": self.results["directory_structure"].get("javascript_files", 0),
            ...
            "route_mismatches": len([m for m in self.results["route_api_mismatches"] 
                                    if m["type"] in ["missing_route", "method_mismatch"]]),
            "unused_routes": len([m for m in self.results["route_api_mismatches"] 
